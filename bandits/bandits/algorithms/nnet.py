...
def build_bnn(x_ph, y_ph, weight_ph, n_train_ph, hps):
  ...
  print(param_names)
  svgd_grad = svgd._svgd_stationary(hps.n_particles, log_likelihood, [svgd_variables[param_name] for param_name in param_names], svgd.rbf_kernel)
  updated_svgd_variables = dict()
  # print(svgd_grad)
  for i in range(len(param_names)):
    updated_svgd_variables[param_names[i]] = tf.stop_gradient(hps.lr * svgd_grad[i][0] + svgd_grad[i][1])
  infer_op = tf.group([svgd_variables[key].assign(updated_svgd_variables[key]) for key in param_names])
  with tf.control_dependencies([infer_op]):
    infer_op = tf.group([svgd_latent[key].assign(svgd_variables[key]) for key in param_names])
